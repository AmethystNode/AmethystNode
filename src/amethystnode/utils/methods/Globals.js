...
/**
 * Require files from AmethystNode
 * @param path {string}
 * @return {*}
 */
global.amethystnode = function(path){
...
void function(){
    function walk(dir){
        SFS.walkDir(dir).forEach(path => {
            if(SFS.basename(SFS.dirname(path)) === "amethystnode" && SFS.isFile(path)) return; //omit Server, AmethystNode
    ...
    function trace(path){
        ...
        path = path.slice(path.lastIndexOf("amethystnode")+1);

        let parent = global.amethystnode;
